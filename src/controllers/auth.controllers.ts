...
import { signupSchema } from "../schema/auth.schema.ts";
import bcrypt from "bcrypt";
import { success, ZodError, z } from "zod";
import { hashGeneratedCode, generateCode, generateRefreshToken, hashRefreshToken} from "../utils/hashing.util.ts";
import generateVerificationEmail from "../emailTemplates/verifycode.template.ts";
import generateWelcomeEmail from "../emailTemplates/welcome.template.ts";
import transporter from "../config/nodemailer.config.ts";
import jwt from "jsonwebtoken";
import { JWT_SECRET, REFRESH_TOKEN_EXPIRES_IN, ACCESS_TOKEN_EXPIRES_IN} from "../config/env.ts";
import { Users } from "../generated/prisma/index";
import { refreshCookieOptions, REFRESH_COOKIE_NAME } from "../config/cookie.config.ts";
    ...
    try{
       ...
       signupSchema.parse({firstName, lastName, email, password})
        ...
        if(checkEmail && checkEmail.is_verified == false){
            ...
            //it is asynchoronous but we do not await it so incase it fails our response still goes through
            //we would update this later to have a worker to retry it incase of failure and alsoa field containing(pending, failed, success) on the db to know what state the mail is in
